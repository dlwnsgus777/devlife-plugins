...
After all cycles complete, **run the full test suite once** with `TEST_CMD` to catch any cross-class regression that the in-cycle scoped runs did not exercise. If anything fails, dispatch a fix sub-agent before proceeding. Then run a final review with a reviewer sub-agent.
