...
|-------|---------|--------|
| `tdd-team` | `2.1.0` | Setup 태스크 분해에 선제 체크 추가 — 목록을 제시하기 전에 태스크마다 「이 테스트를 실패→통과로 만드는 생산 코드 변경」을 한 줄로 답하고, "없음" 또는 "앞선 태스크가 이미 통과시킴"이면 사이클로 만들지 않고 해당 로직을 만드는 태스크에 테스트 메서드로 병합. 기존 `ALREADY_PASSES` 신호는 디스패치를 이미 태운 뒤에 뜨는 백스톱으로 재규정. 실측 세션에서 Red가 될 수 없는 사이클 2개가 전체 에이전트 시간의 41%를 소비한 데 따른 변경 |
| `tdd-team` | `2.2.0` | 테스트 실행 출력 읽기 제약 추가 — RED·GREEN·REFACTOR·FIX 에이전트와 Cycle Reviewer 실험이 전체 콘솔 로그 대신 통과·실패 수, 실패한 테스트 이름, 실패 메시지와 이번 세션 코드의 첫 스택 프레임만 읽도록 지시하고 `| tail -40` 등으로 필터링하도록 안내. 스코프 실행도 프레임워크 기동 로그로 수백 줄을 뱉어 dispatch당 입력을 부풀리고 재시도마다 그 비용을 반복하던 문제 |
| `tdd-team` | `2.3.0` | RED 검증을 2단계로 분리 — Setup에서 `TEST_COMPILE_CMD`를 함께 캡처하고, RED가 컴파일을 먼저 통과시킨 뒤 테스트를 1회만 실행. 새 테스트 클래스는 컴파일 실패가 훨씬 잦은데 이를 전체 테스트 실행으로 확인하던 것을 컴파일 전용 명령으로 대체해 컴파일 에러 1회당 full 실행 한 번을 절약. 아울러 `ALREADY_PASSES` 판정을 기존 생산 코드를 겨냥한 테스트로 한정 — RED가 방금 만든 stub은 정의상 통과할 수 없으므로 판정 대상이 아니며, 이 실행의 목적은 stub이 예외를 던지는지가 아니라 테스트가 메서드에 도달하는지 확인하는 것임을 명시 |
