...
|-------|---------|--------|
| `plan-creator` | `1.8.0` | 깊이 판정(Step 0) 제거. 코드를 한 줄도 보기 전에 Minimal/Standard/Comprehensive를 분류하고 사용자 확인까지 받던 단계였는데, 판정 근거가 없는 데다 실제로 정하던 것들이 서로 다른 질문이었음 — 탐색 반경, 질문 수, 추상화 제안 여부, 리스크 노트. 공통 레이블을 씌운 탓에 각 지점에서 조금씩 틀린 답을 냈고, 특히 탐색 카테고리를 깊이별로 잘라내던 부분은 **요청하지 않은 카테고리는 필요했다는 사실조차 나중에 알 수 없는** 일방향 결정이었음.<br><br>대체 — ⑴ 탐색 반경은 Step 1의 규칙 한 줄로("요청이 지목한 대상을 포함하는 가장 좁은 범위에서 시작하고, 결과가 경계에 닿으면 넓혀 재탐색"), **범위는 잠정·카테고리 목록은 고정**. 8개 카테고리를 항상 전부 요청하고, 생략은 근거 기반 1건만 유지(스펙이 불변성을 확정했으면 6번). ⑵ 질문 수 2~4/5~8/8~12 제거 — 이미 "target, not a cap"이라 구속력이 없었고, 개수는 탐색 결과가 정함. ⑶ 템플릿 섹션 생략 기준을 깊이에서 **Step 1·2가 아무것도 내놓지 못했을 때**로 이동. ⑷ two-case rule에 조건 추가 — **이번 요구사항이 그 축에 케이스를 추가할 때만** 제안. 기존에 "Minimal이면 variation point 스캔 생략"이 대리하던 조건을 직접 기술한 것으로, 버그 수정이 추상화 제안으로 번지는 경로를 정확히 막음. ⑸ brownfield/greenfield 사전 구분 제거 — Step 1이 코드를 보고 내리던 판정과 중복이었음. 262줄 → 228줄. `SKILL.md`(skills/codex)·docs·README 동기화 |
...
| Skill | Version | Change |
|-------|---------|--------|
| `tdd-team` | `2.2.0` | 모델 선택 제거. `## Model Selection` 섹션과 dispatch의 `model` 필드를 모두 삭제하고 전 역할을 세션 기본값으로 실행. 역할별 티어 지정은 Claude/Codex 두 버전이 갈리는 주된 지점이었는데(`"opus"`·`"haiku"` 리터럴 vs "가장 상위 모델"·"저가 티어" 서술), 실질 효과는 Final Reviewer 승격 하나였음. 티어가 사라지면서 죽은 분기가 된 `OVERWHELMED` 1단계("저가 티어 → 기본 티어 재디스패치")도 제거해 재시도를 컨텍스트 축소 1회로 단일화. 두 버전 diff 96줄 → 77줄, 남은 차이는 툴 바인딩(`Agent({})`/`tool_search`, `CLAUDE.md`/`AGENTS.md`, `Edit`·`Write`/`apply_patch`)뿐.<br><br>`Right-Size the Ceremony` 첫 문단에서 중복 문장 1개 제거 — "작은 변경도 큰 변경만큼 취약하다"는 첫 문장의 "변경 크기와 무관하게"를 다시 말한 것. 합리화 차단 근거(구조적 장치·해피패스 편향)는 유지.<br><br>`Handling BLOCKED`을 `Error Handling` 하위 `###`로 이동. Model Selection 삭제 때 하위 절이 `##`로 승격되며 Setup 앞에 그대로 남아, `blocked_reason`·`context.md`·`TASK_DIR`을 전부 전방 참조하는 위치였음 — 정의가 끝난 뒤로 내려가고 예산 표의 `See Handling BLOCKED`가 바로 아래를 가리킴. 티어 재시도 삭제로 남아 있던 고아 번호 `3.`도 산문으로 정리.<br><br>Setup 1의 `TDD_DIR` 정의를 `{PROJECT_ROOT}/.tdd-team` → `.tdd-team`(작업 디렉터리 기준)으로 변경. `PROJECT_ROOT`가 Setup 2에서야 잡히는데 Setup 1이 그 변수로 경로를 정의하고 정작 `mkdir`은 쓰지 않아, 모순을 해명하는 문단이 따로 붙어 있었음 — 정의를 고쳐 문단 자체를 제거하고, 남길 내용(`PROJECT_ROOT`는 에이전트가 `context.md`에서 절대 경로를 필요로 해 잡는 값)만 Setup 2 항목으로 이동.<br><br>Setup 4에서 배칭 규칙을 커버리지 하한 **앞**으로 이동. 하한이 먼저 나오는 탓에 "사이클이 폭증하지 않는다"는 해명 문단이 아직 나오지 않은 배칭 규칙을 가리키고 있었음 — 순서를 바꿔 문단을 삭제하고 비용 모델 한 문장만 하한 끝에 남김. 같은 원인으로 어긋나 있던 방향 지시어 정리: backstop이 control보다 먼저 나와 `the check above`가 실제로는 아래를 가리키던 것을 체크 이름으로 교체하고, `ALREADY_PASSES signal below` → `backstop`으로 변경.<br><br>오케스트레이터 역할 선언을 맨 앞(`Execution Rules`)으로 이동·추가. 정체를 밝히는 문장이 Setup 전체보다 뒤인 디스패치 규칙 블록에만 있어, 100줄 넘게 지시를 받은 뒤에야 자기 역할을 알게 되는 구조였음. 호칭도 정리 — 오케스트레이터를 가리키는 3인칭 2건(`the orchestrator explores…`는 한 문장 안에서 2인칭으로 바뀌던 자리, `enters the orchestrator's context`)을 2인칭으로 통일. `context.md` 템플릿과 디스패치 프롬프트 안의 3인칭은 독자가 에이전트라 그대로 둠.<br><br>스킬 전체 감사로 **서로 반대되는 지시 11건** 제거. `SKILL.md` — ⑴ fix 에이전트에게 findings를 직접 넘기라는 문장(경로만 넘긴다는 494줄·디스패치 표와 정반대) ⑵ 같은 계열인 `with the complete findings list` ⑶ 오케스트레이터가 판단할 수 없는 `REFACTOR — skip if GREEN output is already clean`(스킵은 REFACTOR 소관, 봉투에 판단 필드 없음) ⑷ `The last three lines`가 실제로는 Workspace Rules를 가리키던 것을 항목 이름으로 교체 ⑸ `TEST_CMD` 실행 조건에서 빠져 있던 wide blast radius 케이스 — 조건 나열을 Final Review 한 곳으로 일원화. `references/` — ⑹ `fix-agent`의 "quick, targeted check는 허용"(탐색 금지 규칙을 스스로 무력화) ⑺⑻ `green`·`red`의 "context.md에 없으면 더 읽어라"(같은 조건을 `BLOCKED`로 보내는 규칙과 충돌) ⑼ `refactor`의 skip 분기가 결과 기록(step 6)이 아닌 테스트 실행(step 5)을 가리키던 것 ⑽ `final-reviewer`가 받지 않는 입력 2개. ⑾ `Fixture Pattern` 섹션을 `green-agent.md` → `red-agent.md`로 이관 — 테스트 수정이 금지된 GREEN에게 테스트 데이터 작성법을 지시하고 있었음. `SKILL.md`·`references/` 6개(skills·codex)·docs 동기화 |
...
| Skill | 사유 |
|-------|------|
| `prd-creator` | `spec-creator`와 Purpose·Step 1·출력 구조(Domain Context & Invariants / Sub-task Decomposition / Complexity Guide)가 동일하고 트리거 5개가 충돌 — `spec-creator`가 "PRD 작성해줘"를 legacy 트리거로 이미 흡수하고 있어 통합 |
...
| Skill | Version | Change |
|-------|---------|--------|
| `tdd-team` | `2.1.1` | `context.md`의 Workspace Rules에 불변성 ID 사용 범위를 명시 — `INV-xxx`는 `.tdd-team/` 산출물 안에서만 쓰고 생산·테스트 코드의 주석·이름·메시지에는 넣지 않는다. 계획 문서(`plan-creator`)의 코드 스니핏이 가드 절에 `// INV-001` 주석을 다는 규약이라, 에이전트가 그 태그까지 실제 코드로 옮겨 적는 경로가 있었음. |
...
| Skill | Version | Change |
|-------|---------|--------|
| `tdd-team` | `2.1.0` | 막힌 에이전트가 탐색 대신 에스컬레이션하도록 전환. `TDD_STATUS` 봉투에 `blocked_reason`(`MISSING_FACT`/`OVERWHELMED`) 추가, 디스패치 프롬프트에 스코프 펜스, Setup의 `context.md`는 이번 세션에서 확인한 사실만 기록. 기존 `BLOCKED` 처리는 3단계 모두 "에이전트가 버거워한다"를 전제해 컨텍스트를 줄여 재시도했는데, 정보가 없어 막힌 경우엔 없는 정보를 더 없게 만들어 보내는 셈이라 보고가 탐색보다 불리했음. `MISSING_FACT`는 오케스트레이터가 사실을 구해 `context.md`에 추가한 뒤 줄이지 않은 컨텍스트로 재디스패치하며 예산을 소모하지 않음. 실측 근거: 오케스트레이터가 수 초에 덤프할 스키마를 서브에이전트가 30분에 재구성(오답), import 한 줄 삭제에 37분. |
