...
# cmux — Terminal Control from Codex
...
cmux notify --title "Codex" --subtitle "Waiting" --body "Agent needs input"
...
cmux can be integrated with agent lifecycle hooks to send notifications when tasks complete. The bundled hook reference is Claude Code-specific, so use it only when working inside Claude Code.
