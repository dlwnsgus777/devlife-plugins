...
- `NEEDS_FIX` with Critical or Important > 0 → dispatch a Codex fix sub-agent (see Fix Sub-Agent Dispatch), then re-run the cycle reviewer. Increment `fix_rounds_this_cycle` in `session.md` before each round.
