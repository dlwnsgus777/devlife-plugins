...
description: >
  ...
  as a file. Trigger on "개발인생 codex", "개발인생 codex에게", "개발인생 codex한테",
  "개발인생 codex로", "개발인생 codex에게 해줘", "개발인생 codex한테 시켜",
  "개발인생 codex로 보내", "개발인생 codex에게 물어봐".
  Only trigger when the user explicitly says "개발인생 codex" — the "개발인생" prefix is
  required to distinguish from codex-delegate. Do NOT trigger on bare "codex" mentions
  without the "개발인생" prefix. This skill communicates with a persistent Codex pane
  in cmux (started by devlife-team-starter) and collects the result via a file.
...
This skill works with a Codex instance already running in a cmux pane (typically started by `devlife-team-starter`). It sends the user's task as a prompt, asks Codex to write the result to a file, then watches for that file and presents the result.
...
> "Codex pane을 찾을 수 없습니다. `devlife-team-starter` 스킬로 먼저 Codex pane을 시작하세요."
...
> "Codex pane에 프롬프트를 전송했습니다. 결과 파일을 기다리고 있습니다: `devlifeteam/codex-result-{timestamp}.md`"
...
2. Present the content to the user with a header indicating the source:
   > **Codex Result** (`devlifeteam/codex-result-{timestamp}.md`)
